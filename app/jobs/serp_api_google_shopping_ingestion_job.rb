...
# Job to fetch Google Shopping products via SerpAPI and store as ContentItems.
# Uses the Google Shopping API: https://serpapi.com/google-shopping-api
class SerpApiGoogleShoppingIngestionJob < ApplicationJob
  ...
  def perform(source_id)
    ...
    unless @source.enabled? && @source.google_shopping?
  ...
  def fetch_from_serp_api(api_key, query)
    ...
    # Add configurable location
    location = config_value("location")
    params[:location] = location if location.present?

    # Add configurable country code
    gl = config_value("gl")
    params[:gl] = gl if gl.present?
    ...
    unless response.is_a?(Net::HTTPSuccess)
      raise ExternalServiceError, "SerpAPI Google Shopping request failed: #{response.code} #{response.message}"
    end
  ...
  def process_results(results)
    ...
    shopping_results = results.dig("shopping_results") || []
    max_results = config_value("max_results")&.to_i || 20

    # Limit results to max_results
    shopping_results = shopping_results.first(max_results)

    shopping_results.each do |result|
  ...
  def process_single_result(result, stats)
    ...
    # Update attributes from Google Shopping result
  ...
  def extract_tags(result)
    ...
    # Add source tag
    tags << "source:google_shopping"

    # Tag by merchant/source
    merchant = result["source"]
    tags << "merchant:#{merchant.downcase.gsub(/\s+/, '_')}" if merchant.present?
