...
RSpec.describe AffiliateUrlService, type: :service do
  ...
  describe '#generate_url' do
    context 'with no affiliate template' do
      let(:listing) { build(:listing, tenant: tenant, category: category, affiliate_url_template: nil) }
      let(:service) { described_class.new(listing) }
    ...
    context 'with blank affiliate template' do
      let(:listing) { build(:listing, tenant: tenant, category: category, affiliate_url_template: '') }
      let(:service) { described_class.new(listing) }
    ...
    context 'with ID placeholder' do
      let(:listing) do
        create(:listing,
               tenant: tenant,
               category: category,
               affiliate_url_template: 'https://affiliate.example.com?listing_id={id}')
      end
      let(:service) { described_class.new(listing) }

      it 'replaces {id} with listing ID' do
        result = service.generate_url
        expect(result).to include("listing_id=#{listing.id}")
      end
    end
    ...
    context 'with multiple placeholders' do
      ...
      it 'replaces all placeholders' do
        ...
        expect(result).to include("id=#{listing.id}")
      end
    end
    ...
    context 'with empty affiliate attribution' do
      ...
      it 'returns URL without extra params' do
        result = service.generate_url
        expect(result).to eq("https://affiliate.example.com?url=#{CGI.escape(listing.url_canonical)}")
      end
    end
  ...
  describe '#track_click' do
    ...
    it 'sets the listing association' do
      click = service.track_click(request)
      expect(click.listing).to eq(listing)
    end
    ...
    context 'with unpersisted listing' do
      let(:listing) { build(:listing, tenant: tenant, category: category) }
  ...
  describe '.generate_url_for' do
    ...
    it 'creates a service and calls generate_url' do
      result = described_class.generate_url_for(listing)
  ...
  describe '.track_click_for' do
    ...
    it 'creates a service and calls track_click' do
      expect { described_class.track_click_for(listing, request) }
