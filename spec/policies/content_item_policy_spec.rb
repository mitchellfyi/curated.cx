...
RSpec.describe EntryPolicy, type: :policy do
  ...
  describe "#show?" do
    context "when content item is published" do
      let(:published_item) { create(:entry, :feed, :published, site: site, source: source) }
    ...
    context "when content item is not published" do
      let(:unpublished_item) { create(:entry, :feed, :unpublished, site: site, source: source) }
