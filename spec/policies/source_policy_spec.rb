...
RSpec.describe SourcePolicy, type: :policy do
  ...
  describe "#update?" do
    ...
    context "when source belongs to different tenant" do
      ...
      let(:other_source) { create(:source, :serp_api_google_news, site: other_site) }
  ...
  describe "Scope" do
    let(:scope) { Source.all }
    let(:policy_scope) { described_class::Scope.new(user, scope) }

    let!(:tenant1_source) { source }
    let!(:other_tenant) { create(:tenant) }
    let!(:other_site) { create(:site, tenant: other_tenant) }
    let!(:tenant2_source) { create(:source, :serp_api_google_news, site: other_site) }
