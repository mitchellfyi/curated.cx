...
RSpec.describe "Admin::TaggingRules", type: :request do
  ...
  let!(:taxonomy2) { create(:taxonomy, tenant: tenant2, site: site2, name: "Building") }
  ...
  describe "tenant scoping" do
    before do
      ...
      @tenant2_rule = create(:tagging_rule, taxonomy: taxonomy2, site: site2, pattern: "test2")
    end
    ...
    context "when accessing as admin user" do
      ...
      context "with tenant1 context" do
        ...
        describe "POST /admin/tagging_rules/:id/test" do
          let!(:content_item) do
            source = create(:source, site: site1)
          ...
          it "tests rule against content items" do
            post test_admin_tagging_rule_path(@tenant1_rule)
            expect(response).to have_http_status(:success)
