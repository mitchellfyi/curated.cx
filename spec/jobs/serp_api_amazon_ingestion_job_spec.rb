...
RSpec.describe SerpApiAmazonIngestionJob, type: :job do
  include ActiveJob::TestHelper

  let(:tenant) { create(:tenant) }
  let(:site) { create(:site, tenant: tenant) }
  let(:source) { create(:source, :amazon_search, site: site) }

  describe "#perform" do
    context "happy path" do
      before do
        stub_amazon_search_response
    ...
    context "amazon_domain configuration" do
      let(:source) do
        create(:source, :amazon_search, site: site, config: {
      ...
      it "sends amazon_domain parameter to SerpAPI" do
        described_class.perform_now(source.id)
    ...
    context "when source is disabled" do
      let(:source) { create(:source, :amazon_search, :disabled, site: site) }
    ...
    context "when source is not amazon_search type" do
    ...
    context "when search query is not configured" do
      let(:source) do
        create(:source, :amazon_search, site: site, config: { "api_key" => "test_key" })
      end
