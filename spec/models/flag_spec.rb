...
RSpec.describe Flag, type: :model do
  ...
  describe "validations" do
    ...
    context "uniqueness" do
      it "validates uniqueness of user_id scoped to site and flaggable" do
        ...
        duplicate = build(:flag, flaggable: entry, user: user, site: site)
        expect(duplicate).not_to be_valid
        expect(duplicate.errors[:user_id]).to include("has already flagged this content")
      end

      it "allows same user to flag different entries" do
        create(:flag, flaggable: entry, user: user, site: site)

        other_entry = create(:entry, :feed, site: site, source: source)
        other_flag = build(:flag, flaggable: other_entry, user: user, site: site)
        expect(other_flag).to be_valid
      end

      it "allows different users to flag the same entry" do
        create(:flag, flaggable: entry, user: user, site: site)

        other_user = create(:user)
        other_flag = build(:flag, flaggable: entry, user: other_user, site: site)
        expect(other_flag).to be_valid
      end
    end
    ...
    context "cannot flag own content" do
      ...
      it "allows flagging entries (they have no owner)" do
        # Entries don't have a user association, so this validation doesn't apply
        flag = build(:flag, flaggable: entry, user: user, site: site)
    ...
    context "reviewed_at validation" do
      it "requires reviewed_at when reviewed_by is present" do
        ...
        flag = build(:flag, flaggable: entry, user: user, site: site, reviewed_by: reviewer, reviewed_at: nil)
  ...
  describe "scopes" do
    let!(:pending_flag) { create(:flag, flaggable: entry, user: user, site: site, status: :pending) }
    let!(:reviewed_flag) { create(:flag, :reviewed, flaggable: create(:entry, :feed, site: site, source: source), user: create(:user), site: site) }
    ...
    describe ".for_entries" do
      it "returns flags for entries only" do
        comment = create(:comment, commentable: entry, user: content_owner, site: site)
        comment_flag = create(:flag, :for_comment, flaggable: comment, user: create(:user), site: site)

        expect(Flag.for_entries).to include(pending_flag)
        expect(Flag.for_entries).not_to include(comment_flag)
      end
    end
    ...
    describe ".recent" do
      it "orders by created_at desc" do
        ...
        old_flag = create(:flag, flaggable: create(:entry, :feed, site: site, source: source), user: create(:user), site: site, created_at: 1.hour.from_now)
        new_flag = create(:flag, flaggable: create(:entry, :feed, site: site, source: source), user: create(:user), site: site, created_at: 2.hours.from_now)
    ...
    describe ".by_user" do
      it "returns flags by the specified user" do
        other_user = create(:user)
        other_flag = create(:flag, flaggable: create(:entry, :feed, site: site, source: source), user: other_user, site: site)
  ...
  describe "instance methods" do
    describe "#resolve!" do
      it "marks the flag as reviewed with default action" do
        flag = create(:flag, flaggable: entry, user: user, site: site)
        reviewer = create(:user)
    ...
    describe "#entry?" do
      it "returns true when flaggable is an Entry" do
        flag = build(:flag, flaggable: entry)
        expect(flag.entry?).to be true
      end

      it "returns false when flaggable is not an Entry" do
        comment = create(:comment, commentable: entry, user: content_owner, site: site)
        flag = build(:flag, flaggable: comment)
        expect(flag.entry?).to be false
      end
    end
  ...
  describe "site scoping" do
    ...
    it "scopes queries to current site" do
      flag1 = create(:flag, flaggable: entry, user: user, site: site)

      Current.site = other_site
      flag2 = create(:flag, flaggable: other_entry, user: create(:user), site: other_site)
    ...
    describe "after_create" do
      ...
      context "auto-hide threshold" do
        ...
        it "hides content when threshold is reached" do
          hideable_content = create(:entry, :feed, site: site, source: source)
